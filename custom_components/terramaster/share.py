...
def share_connection_urls(
    ...
    authenticated_host = (
        f"{encoded_username}@{url_host}" if encoded_username else url_host
    )

    urls: dict[str, str] = {}
    if "smb" in share.protocols:
        urls["smb"] = f"smb://{authenticated_host}/{encoded_name}"
    if "nfs" in share.protocols:
        urls["nfs"] = f"nfs://{url_host}{quote(share.path, safe='/')}"
    if "afp" in share.protocols:
        urls["afp"] = f"afp://{authenticated_host}/{encoded_name}"
    return urls
...
def share_page_url(
    ...
    parameters = {"entry_id": entry_id, "token": token}
...
class TerraMasterShareView(HomeAssistantView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        all_shares_title = "Partages TerraMaster" if is_french else "TerraMaster shares"
        prompt = (
        ...
        open_label = "Ouvrir avec" if is_french else "Open with"
        user_label = "Utilisateur proposé" if is_french else "Suggested username"
        hidden_label = "Partage masqué" if is_french else "Hidden share"
        recycle_label = "Corbeille activée" if is_french else "Recycle bin enabled"

        labels = {"smb": "SMB / CIFS", "nfs": "NFS", "afp": "AFP"}
        protocol_icons = {"smb": "▣", "nfs": "◆", "afp": "●"}
        detail_labels = (
            {
                "volume": "Volume",
                "raid": "RAID",
        ...
        for share in shares:
            urls = share_connection_urls(host, share, username)
            protocol_badges = "".join(
                (
                    f'<span class="badge protocol-{protocol}">'
                    f'<span aria-hidden="true">{protocol_icons[protocol]}</span> '
                    f"{escape(labels[protocol])}</span>"
                )
            ...
            if not protocol_badges:
                protocol_badges = (
                    ...
                    f"{escape(unavailable)}</span>"
                )

            property_badges: list[str] = []
            if share.hidden:
                property_badges.append(
                    f'<span class="badge secondary">{escape(hidden_label)}</span>'
                )
            if share.recycle_bin:
                property_badges.append(
                    f'<span class="badge secondary">{escape(recycle_label)}</span>'
                )
            badges = protocol_badges + "".join(property_badges)

            links = "\n".join(
                (
                    f'<a class="button protocol-{protocol}" '
                    f'href="{escape(url, quote=True)}">'
                    '<span class="protocol-icon" aria-hidden="true">'
                    f"{protocol_icons[protocol]}</span>"
                    f"<span>{escape(open_label)} {escape(labels[protocol])}</span>"
                    "</a>"
                )
                for protocol, url in urls.items()
            )
            if not links:
                links = f'<p class="unavailable">{escape(unavailable)}</p>'
            ...
            username_note = (
                '<p class="username">'
                f"{escape(user_label)} : <strong>{escape(username)}</strong></p>"
            )
            sections.append(
                "<section>"
                f'<div class="share-heading"><h2>{escape(share.name)}</h2></div>'
                f'<p class="path"><code>{escape(share.path)}</code></p>'
                f'<div class="badges">{badges}</div>'
                f"{detail_list}{username_note}"
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    section {{
      background: color-mix(in srgb, Canvas 96%, #03a9f4 4%);
    ...
    .protocol-icon {{ font-size: 1.1rem; }}
